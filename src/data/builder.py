...
def _build_ddad_raw(split: str, cfg: Any, manifest_paths: list[str] | None = None):
    ...
    return DdadRawDataset(
        DdadRawConfig(
            ...
            split=str(split),
            camera=str(cfg.get_path("data.ddad.camera", "CAMERA_01")),
            clip_frames=_clip_frames(cfg),
            image_size=_image_size(cfg),
            queries_per_clip=_queries_per_clip(cfg),
            hard_query_ratio=_hard_query_ratio(cfg),
            ...
            **_query_timestep_delta_kwargs(cfg),
            training=(split == "train"),
            max_scenes=_optional_int(cfg.get_path("data.ddad.max_scenes", None)),
            max_lidar_points_per_frame=int(cfg.get_path("data.ddad.max_lidar_points_per_frame", 20000)),
            depth_consistency_abs_m=float(cfg.get_path("data.ddad.depth_consistency_abs_m", 0.5)),
